...
The code is organized as a Python package with a modular architecture. Core application logic (UI, state management, etc.) is kept separate from the STT engine implementations. Each speech recognition engine is a self-contained sub-package within the `dictatux/engines/` directory, which makes the system extensible and easy to maintain.
...
The Whisper Local integration is implemented in the `dictatux/engines/whisper_local/` package and provides a high-accuracy, fully offline speech recognition solution using `faster-whisper`.
...
The Vosk Local integration is implemented in the `dictatux/engines/vosk_local/` package and provides a lightweight, privacy-focused offline speech recognition solution.
...
The Whisper Docker integration is implemented in the `dictatux/engines/whisper/` package and runs OpenAI's Whisper ASR in a Docker container as a REST API service.
...
The Google Cloud Speech integration is implemented in the `dictatux/engines/google/` package and uses Google's enterprise-grade speech recognition API with gRPC streaming.
...
The application uses Qt resource files (`.qrc`) to bundle icons and other assets directly into the Python package. The compiled resource file is `dictatux/dictatux_rc.py`.

### When to Recompile Resources

You must recompile resources when:
- Adding or modifying icons in the `icons/` directory
- Changing the `dictatux.qrc` file structure
- Updating icon files (PNG, SVG)

### Compiling Resources

Use the provided Docker-based script (does not require system Qt tools):

```bash
./compile_resources.sh
```

This script uses Docker to run `pyrcc6` inside an Ubuntu container, avoiding the need to install Qt development tools on your system.

**Note:** The `dictatux_rc.py` file should be committed to the repository after regeneration.

## Translations

The application interface is available in multiple languages. The translation source files (`.ts`) are located in the `dictatux/translations/` directory.
...
# Or using local Qt tools
./.venv/lib/python3.12/site-packages/qt6_applications/Qt/bin/lrelease dictatux/translations/*.ts
```

### Docker-Based Qt Tools

Both resource compilation and translation management use Docker to avoid requiring system-wide Qt development tools installation. The scripts automatically download and run the necessary Qt tools inside containers.

**Benefits:**
- No need to install `qt6-base-dev-tools` or similar packages
- Consistent tool versions across different development environments
