...
def stub_pulseaudio(monkeypatch):
    monkeypatch.setattr(
        "dictatux.audio_recorder.get_audio_devices",
...
def test_advanced_dialog_creates_dynamic_tabs(qt_app):
    ...
    assert "Google Cloud Speech" in tab_texts
    assert "OpenAI Realtime" in tab_texts
...
def test_manage_models_action_updates_path(qt_app, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "dictatux.dialogs.launch_model_selection_dialog",
        fake_launch,
    )
