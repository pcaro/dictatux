...
REAL_API_RESPONSES: List[MockStreamingResponse] = [
    ...
    MockStreamingResponse([
        MockRecognitionResult(
            alternatives=[MockRecognitionAlternative(
...
def create_runner(input_simulator: Callable[[str], None], **kwargs) -> "GoogleCloudSpeechProcessRunner":
    """
...
class TestGoogleCloudSpeechCurrentBehavior:
    ...
    def test_only_finals_generate_calls(self):
        ...
        assert mock.calls[0] == REAL_API_RESPONSES[2].results[0].alternatives[0].transcript
        assert mock.calls[1] == REAL_API_RESPONSES[4].results[0].alternatives[0].transcript
...
class TestGoogleCloudSpeechWithPartials:
    ...
    def test_interim_replaced_by_final(self):
        ...
        assert mock.calls[1] == " intentando realizar un párrafo bastante largo para ver si tenemos"
        ...
        assert mock.calls[3] == "Eso es la prueba. Ahora mismo termino el párrafo y voy a ver"
        # Call 4: The second final starts with a space and changes punctuation, 
        # so there's no common prefix. We expect 60 backspaces.
        assert mock.calls[4] == "\b" * len("Eso es la prueba. Ahora mismo termino el párrafo y voy a ver")
        # Call 5: The full second final is typed
        assert mock.calls[5] == " Eso es la prueba, ahora mismo termino el párrafo y voy a ver si transcribe."
