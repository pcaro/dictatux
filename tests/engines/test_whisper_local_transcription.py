...
from dictatux.engines.whisper_local.inference_backend import (
    WhisperInferenceBackend,
    ContextManager,
)
...
def test_whisper_local_filters_hallucinations(whisper_backend):
    ...
    segments = [
        ...
        FakeSegment(
            text=" Gracias por ver.", no_speech_prob=0.9
        ),  # hallucination > 0.6
...
def test_whisper_local_context_manager(whisper_backend):
    ...
    whisper_backend._model.transcribe.return_value = (
        [FakeSegment(text="Primera frase.", no_speech_prob=0.1)],
        FakeInfo(),
    )
    ...
    whisper_backend._model.transcribe.return_value = (
        [FakeSegment(text=" Segunda frase.", no_speech_prob=0.1)],
        FakeInfo(),
    )
    whisper_backend.transcribe(b"\x00" * 100)
