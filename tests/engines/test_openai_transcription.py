...
def test_openai_transcription_completed():
    ...
    runner._controller.emit_transcription.assert_called_with(
        "Esto es una prueba de dictado."
    )
...
def test_openai_transcription_partial_deltas():
    ...
    for d in deltas:
        msg = {"type": "conversation.item.input_audio_transcription.delta", "delta": d}
        runner._on_message(runner._ws, json.dumps(msg))
...
def test_openai_error_message():
    ...
    msg = {"type": "error", "error": {"message": "Invalid API key"}}
    runner._on_message(runner._ws, json.dumps(msg))
...
def test_openai_response_output_text_delta_accumulation():
    ...
    runner._on_message(
        runner._ws,
        json.dumps({"type": "response.completed", "response": {"id": "resp_001"}}),
    )
