...
def test_whisper_local_create_engine():
    ...
    with (
        patch("dictatux.engines.whisper_local.inference_backend.WhisperModel"),
...
def test_whisper_local_full_lifecycle():
    ...
    # Mock backend, pipeline, capture and VAD to test runner logic in isolation
    with (
        patch(
            "dictatux.engines.whisper_local.runner.WhisperInferenceBackend"
        ...
        max_wait = 2.0
        start_t = time.time()
        while controller.state == WhisperLocalState.LOADING and (
            time.time() - start_t < max_wait
        ):
            time.sleep(0.1)
