...
def test_whisper_local_plugin_registration():
    ...
    from dictatux.stt_factory import get_available_engines
...
def test_whisper_local_create_engine():
    ...
    from dictatux.engines.whisper_local.settings import WhisperLocalSettings
    
    settings = WhisperLocalSettings()
    
    # Mock model and VAD to avoid loading heavy deps
    with patch('dictatux.engines.whisper_local.inference_backend.WhisperModel'), \
         patch('dictatux.vad_processor.SileroVADProcessor._load_model'):
...
def test_whisper_local_full_lifecycle():
    ...
    from dictatux.engines.whisper_local.runner import WhisperLocalRunner
    from dictatux.engines.whisper_local.controller import WhisperLocalController
    from dictatux.engines.whisper_local.settings import WhisperLocalSettings
    from dictatux.engines.whisper_local.controller import WhisperLocalState
    ...
    with patch('dictatux.engines.whisper_local.runner.WhisperInferenceBackend') as mock_backend_class, \
         patch('dictatux.engines.whisper_local.runner.AudioPipeline') as mock_pipeline_class, \
         patch('dictatux.engines.whisper_local.runner.AudioCapture') as mock_capture_class:
...
def test_whisper_local_context_manager():
    ...
    from dictatux.engines.whisper_local.inference_backend import ContextManager
