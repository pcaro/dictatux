...
def test_refresh_timeout_cancels_after_success(monkeypatch, qt_app):
    ...
    assert manager._runner is created_runners[-1]
    assert (
        manager._refresh_timeout_timer and not manager._refresh_timeout_timer.isActive()
    )


def test_circuit_breaker_switches_to_fallback(monkeypatch, qt_app):
    settings = DummySettings()
    # Use whisper-docker which has vosk-local as first fallback
    ...
    # Should fallback to vosk-local (first available local engine)
    assert manager.active_engine_type == "vosk-local"
    assert created_engines[-1] == "vosk-local"
    assert manager._failure_count == 0


def test_circuit_breaker_calls_fatal_without_fallback(monkeypatch, qt_app):
    """Test that on_fatal is called when all fallback engines are exhausted."""
    settings = DummySettings()
    # Use a mock settings that reports only one engine available
    ...
    # Exhaust all fallback engines by cycling through failures
    # With dynamic fallback chain, openai-realtime has multiple fallbacks
    # We need to fail enough times to exhaust them all
    for _ in range(10):  # Fail many times to exhaust all fallbacks
        if fatal_called:
            break
    ...
    assert len(created_engines) > 1  # Should have tried multiple engines


def test_circuit_breaker_restores_primary_after_window(monkeypatch, qt_app):
    """Test that primary engine is restored after circuit breaker window."""
    settings = DummySettings()
    # Use whisper-docker which has vosk-local as fallback
    settings.sttEngine = "whisper-docker"
    manager = EngineManager(settings, max_retries=1, retry_delay_ms=1)
    ...
    manager.handle_exit(1)
    # Should fallback to vosk-local
    assert manager.active_engine_type == "vosk-local"
    fallback_runner = manager._runner
    assert fallback_runner is not None

    # Simulate circuit breaker window elapsed
    manager._circuit_open_until = datetime.now() - timedelta(seconds=1)
    manager.handle_exit(0)

    # Should restore to primary engine
    assert manager.active_engine_type == "whisper-docker"
    assert manager._temporary_engine is None
    assert created_engines[-1] == "whisper-docker"


def test_old_controller_exit_after_refresh_doesnt_affect_new_engine(
    monkeypatch, qt_app
):
    ...
    assert manager._failure_count == 0
    assert len(old_controller._exit_listeners) == 1, (
        "Old controller should have exit listener"
    )
    ...
    assert manager._failure_count == 0
    assert len(new_controller._exit_listeners) == 1, (
        "New controller should have exit listener"
    )
