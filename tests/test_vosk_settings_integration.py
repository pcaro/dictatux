...
def test_settings_write_vosk_config():
    ...
    with patch("dictatux.settings.QSettings") as mock_qsettings:
        ...
        saved_values = {
            call[0][0]: call[0][1] for call in mock_backend.setValue.call_args_list
        }
...
def test_vosk_engine_creation_via_factory():
    ...
    with (
        patch("dictatux.settings.QSettings") as mock_qsettings,
        patch("vosk.Model"),
        patch("dictatux.engines.vosk_local.runner.SileroVADProcessor"),
    ):
        mock_backend = MagicMock()
        mock_backend.value.return_value = ""
        mock_backend.beginReadArray.return_value = 0
