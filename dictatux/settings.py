...
class Settings:
    ...
    def __init__(self, backend: Optional[QSettings] = None) -> None:
        if backend is None:
            if os.environ.get("PYTEST_CURRENT_TEST"):
                ...
                backend = QSettings("Dictatux", "Dictatux")
    ...
    def get_engine_settings(self, engine_type: Optional[str] = None) -> EngineSettings:
        """
        ...
        Raises:
            ValueError: If engine settings validation fails (e.g., invalid port).
        """
        requested_type = engine_type or self.sttEngine
        canonical_type = normalize_engine_name(requested_type)

        try:
            plugin = get_plugin(canonical_type)
            return plugin.load_settings_from_app(self)
        except PluginNotFoundError:
            # Fallback to base settings for unknown engines
            return EngineSettings(
                engine_type=canonical_type,
                device_name=self.deviceName,
            )

    def update_from_dataclass(self, engine_settings: EngineSettings) -> None:
        """
