...
class SystemTrayIcon(QSystemTrayIcon):
    ...
    def dictation_controller(self) -> Optional[STTController]:
        ...
        return (
            self._engine_manager.controller
            if hasattr(self, "_engine_manager")
            else None
        )
    ...
    def __init__(
        ...
        self.state_machine.on_state = lambda state: self._apply_state(
            state.icon_state, state.dictating, state.suspended
        )
        ...
        self.micro = get_icon("audio-input-microphone", ":/icons/dictatux/24/micro.png")
    ...
    def _register_global_shortcuts(self):
        ...
        if self.settings.beginShortcut:
            success = self.ipc.register_global_shortcut(
                "begin", self.settings.beginShortcut, self.begin
            )
            if success:
                logging.info(
        ...
        if self.settings.endShortcut:
            success = self.ipc.register_global_shortcut(
                "end", self.settings.endShortcut, self.end
            )
            if success:
                logging.info(
        ...
        if self.settings.toggleShortcut:
            success = self.ipc.register_global_shortcut(
                "toggle", self.settings.toggleShortcut, self.controller_toggle
            )
            if success:
                logging.info(
                    f"Global shortcut registered: {self.settings.toggleShortcut} -> toggle"
                )
            else:
        ...
        if self.settings.suspendShortcut:
            success = self.ipc.register_global_shortcut(
                "suspend", self.settings.suspendShortcut, self.suspend
            )
            if success:
                logging.info(
                    f"Global shortcut registered: {self.settings.suspendShortcut} -> suspend"
                )
            else:
        ...
        if self.settings.resumeShortcut:
            success = self.ipc.register_global_shortcut(
                "resume", self.settings.resumeShortcut, self.resume
            )
            if success:
                logging.info(
                    f"Global shortcut registered: {self.settings.resumeShortcut} -> resume"
                )
            else:
    ...
    def exit(self) -> None:
        ...
        logging.debug("Exiting Dictatux...")
        if self.dictating:
            self.stop_dictate()
        # Cleanup resources
        remove_pid_file()
    ...
    def show_config_dialog(self) -> None:
        adv_window = AdvancedUI(
            self.settings, reset_context_callback=self._handle_reset_context
        )
        ...
        # Interface Language
        index = adv_window.ui.interface_language_cb.findData(
            self.settings.interfaceLanguage
        )
        if index >= 0:
        ...
        while True:
            ...
            from dictatux.validators import validate_command_exists
            ...
            if general_warnings:
                ...
                general_tab_index = adv_window.ui.tabWidget.indexOf(
                    adv_window.ui.general_tab
                )
                if general_tab_index >= 0:
                    tab_text = adv_window.ui.tabWidget.tabText(general_tab_index)
                    if not tab_text.startswith("⚠️ "):
                        adv_window.ui.tabWidget.setTabText(
                            general_tab_index, f"⚠️ {tab_text}"
                        )
        ...
        if True:  # Keep indentation consistent
            ...
            logging.info(
                f"Saving device - currentData: {current_data}, currentText: {current_text}"
            )
            ...
            self.settings.beginShortcut = (
                adv_window.beginShortcut.keySequence().toString()
            )
            self.settings.endShortcut = adv_window.endShortcut.keySequence().toString()
            self.settings.toggleShortcut = (
                adv_window.toggleShortcut.keySequence().toString()
            )
            self.settings.suspendShortcut = (
                adv_window.suspendShortcut.keySequence().toString()
            )
            self.settings.resumeShortcut = (
                adv_window.resumeShortcut.keySequence().toString()
            )
            ...
            from dictatux.engine_plugin import get_all_engine_ids
            ...
            for engine_id in get_all_engine_ids():
                ...
                try:
                    ...
                    logging.debug(
                        "Failed to update settings for %s: %s", engine_id, exc
                    )
            ...
            self._engine_manager.refresh_engine(
                stop_callback=self.stop_dictate, poll_timer=self.dictation_timer
            )
            self._update_tooltip()
