...
class IPCDBus(IPCManager):
    ...
    def __init__(self, app_id: str = "dictatux"):
        ...
        self._signal_connected = (
            False  # Track if we've connected to KGlobalAccel signal
        )
    ...
    def is_running(self) -> bool:
        ...
        iface = QDBusInterface(self.dbus_service, self.dbus_path, "", bus)
    ...
    def start_server(self) -> bool:
        ...
        if not bus.registerObject(
            self.dbus_path, self, QDBusConnection.RegisterOption.ExportAllSlots
        ):
            error_msg = bus.lastError().message()
    ...
    def send_command(self, command: str) -> bool:
        ...
        if not iface.isValid():
            logging.error(
                f"Cannot connect to D-Bus service: {bus.lastError().message()}"
            )
            return False
    ...
    def supports_global_shortcuts(self) -> bool:
        ...
        iface = QDBusInterface(
            "org.kde.kglobalaccel", "/kglobalaccel", "org.kde.KGlobalAccel", bus
        )
    ...
    def register_global_shortcut(
        self, action: str, shortcut: str, callback: Callable
    ) -> bool:
        """
        ...
        logging.debug(
            f"[KGlobalAccel] Parsed shortcut '{shortcut}' to key codes: {key_codes}"
        )
        ...
        action_id = [
            ...
            "Dictatux",  # Component friendly name
        ...
        logging.debug(
            f"[KGlobalAccel] Registering shortcut: actionId={action_id}, combined_key={combined_key}, flags={flags}"
        )
        ...
        if not self._context_activated:
            logging.debug(
                f"[KGlobalAccel] Activating global shortcut context for component '{component}'"
            )
            try:
                result = subprocess.run(
                    [
                ...
                if result.returncode == 0:
                    ...
                    logging.warning(
                        f"[KGlobalAccel] Context activation failed: {result.stderr}"
                    )
            except Exception as e:
        ...
        if component_reply.errorName():
            logging.error(
                f"[KGlobalAccel] getComponent failed: {component_reply.errorMessage()}"
            )
            component_path = f"/component/{component}"
            logging.debug(
                f"[KGlobalAccel] Falling back to assumed path: {component_path}"
            )
        else:
            component_path = component_reply.arguments()[0]
            logging.debug(
                f"[KGlobalAccel] Got component path from KGlobalAccel: {component_path}"
            )
        ...
        logging.info(
            f"[KGlobalAccel] Global shortcut '{shortcut}' registered for action '{action}'"
        )
        return True
    ...
    def _on_global_shortcut(self, component: str, unique_name: str, timestamp: int):
        ...
        """
        logging.debug(
            f"[KGlobalAccel] *** SHORTCUT SIGNAL RECEIVED *** component='{component}', unique_name='{unique_name}', timestamp={timestamp}"
        )
        ...
        if unique_name in self.shortcuts:
            ...
            logging.warning(
                f"[KGlobalAccel] No callback registered for shortcut: {unique_name}"
            )
            logging.debug(
