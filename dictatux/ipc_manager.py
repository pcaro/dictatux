...
# -*- coding: utf-8 -*-
"""
Inter-Process Communication (IPC) Manager for Dictatux

Provides an abstraction layer for communication between Dictatux instances
...
def create_ipc_manager(app_id: str = "dictatux") -> IPCManager:
    """
    ...
    try:
        ...
        if bus.isConnected():
            ...
            logging.info(
                "D-Bus session bus not available, falling back to QLocalServer"
            )
    except ImportError as e:
        logging.info(f"D-Bus module not available ({e}), falling back to QLocalServer")
    except Exception as e:
        logging.warning(
            f"Failed to check D-Bus availability ({e}), falling back to QLocalServer"
        )

    # Fallback to QLocalServer
    from dictatux.ipc_localsocket import IPCLocalSocket
