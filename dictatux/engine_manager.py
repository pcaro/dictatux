...
from dictatux.engine_plugin import normalize_engine_name
...
def _build_fallback_chain() -> Dict[str, List[str]]:
    """Build dynamic fallback chain based on registered plugins.

    Returns:
        Dictionary mapping engine names to their fallback chain.
        Fallbacks are ordered by preference: local engines first, then cloud.
    """
    from dictatux.engine_plugin import get_all_engine_ids

    all_engines = list(get_all_engine_ids())
    ...
    def get_fallbacks(engine: str) -> List[str]:
        ...
        # Priority 1: Local engines (most reliable, offline)
        for e in local_engines:
            if e != engine and e in all_engines:
                fallbacks.append(e)

        # Priority 2: Docker engines
        for e in docker_engines:
            if e != engine and e in all_engines:
                fallbacks.append(e)

        # Priority 3: Cloud engines (least reliable, requires internet)
    ...
    return {engine: get_fallbacks(engine) for engine in all_engines}


# Static fallback chain kept for backwards compatibility
# Use _build_fallback_chain() for dynamic generation
...
class EngineManager(QObject):
    ...
    def _sync_user_engine(self) -> None:
        ...
        if configured != self._user_engine:
            ...
            # Use dynamically built fallback chain
            dynamic_fallbacks = _build_fallback_chain().get(self._user_engine, [])
            self._fallback_chain = [
                self._user_engine,
                *[
                    engine
                    for engine in dynamic_fallbacks
                    if engine != self._user_engine
                ],
            ]
