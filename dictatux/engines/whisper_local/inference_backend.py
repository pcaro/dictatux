...
import numpy as np
from dictatux.inference_backend import InferenceBackend
...
class WhisperInferenceBackend(InferenceBackend):
    ...
    def load_model(self, model_path: str, **kwargs) -> None:
        ...
        logging.info(
            f"Loading Whisper model '{model_path}' on {self._device} ({self._compute_type})..."
        )
    ...
    def transcribe(self, audio: bytes) -> str:
        ...
        logging.debug(
            f"Transcribing with Whisper (language={self._language}, context_len={len(context) if context else 0})..."
        )
        ...
        for segment in segments:
            if segment.no_speech_prob > 0.6:
                logging.debug(
                    f"Filtering likely hallucination: '{segment.text}' (prob: {segment.no_speech_prob:.2f})"
                )
                continue
