...
class WhisperLocalRunner:
    ...
    def start(self, command: Sequence[str] = None, env: Optional[dict] = None) -> bool:
        ...
        if not self._is_model_cached(self._settings.model_size):
            ...
            self._controller._emit_output(
                f"Downloading Whisper '{self._settings.model_size}' model from HuggingFace..."
            )
    ...
    def _load_model_task(self):
        ...
        try:
            self._backend.load_model(
                ...
                auto_reset_seconds=30.0
                if self._settings.auto_reset_context
                else 999999,
