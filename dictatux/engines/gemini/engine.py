...
class GeminiLivePlugin(EnginePlugin):
    ...
    def apply_to_settings(
        self, app_settings: "Settings", engine_settings: GeminiSettings
    ) -> None:
    ...
    def load_settings_from_app(self, app_settings: "Settings") -> GeminiSettings:
        """Create GeminiSettings from application Settings."""
        return GeminiSettings(
            engine_type=self.name,
            device_name=app_settings.deviceName,
            api_key=app_settings.geminiApiKey,
            model=app_settings.geminiModel,
            language_code=app_settings.geminiLanguageCode,
            sample_rate=app_settings.geminiSampleRate,
            channels=app_settings.geminiChannels,
            vad_enabled=app_settings.geminiVadEnabled,
            vad_threshold=app_settings.geminiVadThreshold,
        )
