...
from dictatux.threaded_runner import ThreadedInferenceRunner
from dictatux.audio_pipeline import AudioPipeline, AudioCapture, AudioBuffer
from dictatux.vad_processor import (
    SileroVADProcessor,
    WebRTCVADProcessor,
    RMSVADProcessor,
)
...
class VoskLocalRunner:
    ...
    def __init__(
        ...
        self._partial_handler = (
            PartialTextHandler(self._input_simulator)
        ...
        formatter = (
            TextFormatter(locale=settings.locale) if settings.text_formatting else None
        )
    ...
    def _create_vad(self):
        ...
        try:
            ...
            logging.error(
                f"Requested VAD '{self._settings.vad_type}' not available: {e}. Falling back to RMS."
            )
    ...
    def start(self, command: Sequence[str], env: Optional[dict] = None) -> bool:
        ...
        def load_async():
            try:
                ...
                self._backend.load_model(
                    ...
                    partial_callback=self._on_partial
                    if self._settings.partial_results
                    else None,
                )
        ...
        loading_thread = threading.Thread(
            target=load_async, daemon=True, name="VoskLoadThread"
        )
